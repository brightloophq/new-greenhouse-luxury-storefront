...
- [ ] **Wholesale application email notification** — send an internal "New Wholesale Application" email (business name, contact, email, phone, CRA/TRN, business type) when a wholesale profile is submitted. **Blocked:** no email provider is configured (see `08` — automated verification was removed; onboarding is now a manual review). Needs the owner to choose an email provider. (The automated-verification programme was removed 2026-07.)
