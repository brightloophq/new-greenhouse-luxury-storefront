...
- `npx vitest run` — 553 tests across 37 files (baseline @ `abb19f4`). Many are **source-string regression** tests (assert a route/component still delegates to the real Shopify logic and never reimplements cart/variant/price).
- `npm run typecheck`, `npx eslint`, `npm run build` must all be green before commit. Use `npm run typecheck` (`react-router typegen && tsc --noEmit`), **not** bare `npx tsc --noEmit` — the route `+types` and `virtual:react-router/server-build` typings are generated by `react-router typegen`, so bare `tsc` fails on a fresh tree with spurious "Cannot find module" errors.
